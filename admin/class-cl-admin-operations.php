...
class ContactListAdminOperations {
  ...
  public function operations() {
    ...
    } elseif (isset($_POST) && isset($_POST['_contact_list_empty_search_log']) && isset($_REQUEST['_wpnonce']) && is_super_admin()) {

      $wp_nonce = sanitize_text_field( $_REQUEST['_wpnonce'] );

      if ( wp_verify_nonce($wp_nonce, '_contact-list-empty-search-log')) {

        global $wpdb;
        $delete = $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}contact_list_search_log");

        $goto_url = get_admin_url(null, './edit.php?post_type=' . CONTACT_LIST_CPT . '&page=contact-list-search-log&search_log_emptied=1');
        ...
        $goto_url = get_admin_url(null, './edit.php?post_type=' . CONTACT_LIST_CPT . '&page=contact-list-search-log&search_log_emptied_error=1');
