...
function updateContactMarkup($contact_id) {
...
//        '_cl_description'     => isset($_POST['_cl_description']) ? balanceTags(wp_kses_post($_POST['_cl_description'], 1)) : '',
      ),
    );
...
//    $html .= '<input name="email" class="input-email" />';
    ...
    if (!isset($s['pf_hide_additional_info']) && 0) {  
      ...
      $settings = array('media_buttons' => false, 'teeny' => true, 'wpautop' => false, 'textarea_rows' => 16, 'quicktags' => false);
      ob_start();
